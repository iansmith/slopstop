...
**Status:** Scaffolding stage. The directory structure is in place; the first child ticket of [BILL-28](https://github.com/iansmith/ticket-plugin/issues/28) lands `pyproject.toml`, the embed/rerank/db modules, the pytest harness, and the initial `main.py`.
...
```bash
make rag-build    # from the repo root — builds ticket-plugin/rag:latest
...
- [BILL-28](https://github.com/iansmith/ticket-plugin/issues/28) — application-layer umbrella
