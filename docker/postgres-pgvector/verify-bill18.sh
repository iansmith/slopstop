...
# Check 6 — `make rag-build` succeeds AND tags both slopstop:latest
# and slopstop:<git-sha>.
build_ok() {
    make rag-build > /tmp/bill18-build.log 2>&1 || return 1
    docker image inspect "slopstop:latest"    >/dev/null 2>&1 || return 1
    docker image inspect "slopstop:$GIT_SHA"  >/dev/null 2>&1 || return 1
}
check "make rag-build produces slopstop:latest AND :<git-sha>" build_ok
...
size_match() {
    ...
    actual_bytes=$(docker image inspect slopstop:latest --format '{{.Size}}' 2>/dev/null) || return 1
    python3 -c "
...
check "README image size within +/-10% of actual slopstop:latest" size_match
...
# Check 10 — `make rag-clean` removes both slopstop tags and any
# leftover smoke-test container.
clean_ok() {
    make rag-clean > /tmp/bill18-clean.log 2>&1 || return 1
    # No slopstop images of any tag.
    if docker images --format '{{.Repository}}' | grep -qx 'slopstop'; then
        return 1
    fi
...
check "make rag-clean removes slopstop images and test containers" clean_ok
