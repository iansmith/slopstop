...
**Linear complexity derivation (harvester budget).** Linear scores a query as *0.1 pt/property + 1 pt/object, with each connection multiplying its children by its `first:` argument* (default 50). The harvester's per-issue selection — the 5 core scalars (`id, identifier, title, description, url`) plus the BILL-51 metadata fields (`state`, `assignee`, `creator`, `cycle`, `labels(first: 50)`, and the priority/timestamp scalars) — sums to **60.6 pt** (field-by-field breakdown in `linear.py`'s `_ISSUE_SCALAR_COMPLEXITY` comment), plus `comments(first: 100)`. Each comment node costs `1 (comment object) + 0.3 (id, body, createdAt) + 1 (nested user object) + 0.1 (user.name) = 2.4 pt`, so the comments connection is `100 × 2.4 = 240 pt`. Per issue ≈ `60.6 + 240 = 300.6 pt`; the enclosing `issues(first: N)` connection adds 1 pt/issue for the issue object itself, giving **≈ 301.6 pt per issue returned**. Therefore:

- **`sync_ticket`** = `issues(first: 1)` ≈ **~302 pts/call**. The 2,500 req/hr request limit binds first (2,500 calls ≈ 755K pts, under 3M) → ceiling ≈ **~2,500 tickets/hr**.
- **`sync_recent`** = `issues(first: 32)` ≈ **~9,651 pts/page**. The 3M-pts/hr complexity limit binds first (3,000,000 ÷ 9,651 ≈ 311) → ceiling ≈ **~311 pages/hr ≈ 9,950 tickets/hr**.
- **Single-query cap:** at `comments(first: 100)`, a batch of N issues costs `N × 301.6`; the 10,000-pt per-query ceiling caps the batch at **⌊10000 / 301.6⌋ = 33 issues**. The harvester uses **`first: 32`** (`LINEAR_BATCH_SIZE`) for a small safety margin — comment depth and batch size are not independently tunable, so dropping batch size is the lever if the per-query estimate proves low.
