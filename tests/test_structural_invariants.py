...
class TestPhase0MarkerContract:
    ...
    def test_writer_specifies_the_marker(self):
        assert PHASE0_MARKER in PHASE0_WRITER.read_text(), (
            f"{PHASE0_WRITER.relative_to(REPO_ROOT)} must specify the literal "
            f"marker {PHASE0_MARKER!r} that :plan writes for a prose-only change"
        )

    @pytest.mark.parametrize("reader", PHASE0_READERS, ids=lambda p: p.name)
    def test_readers_match_the_same_marker(self, reader):
        assert PHASE0_MARKER in reader.read_text(), (
            f"{reader.relative_to(REPO_ROOT)} must name the same literal marker "
            f"{PHASE0_MARKER!r} that plan-phase0-mechanics.md writes. These two "
            "sides are a string contract — a paraphrase on either side silently "
            "disables the prose-only exemption."
        )


# --------------------------------------------------------------------------
# Portability of shell embedded in the skills
# --------------------------------------------------------------------------


class TestEmbeddedShellPortability:
    """The skills ship shell snippets that agents run verbatim, on macOS by
    default. A GNU/PCRE-only construct works on the author's Linux box and exits
    2 with empty stdout on BSD grep — which several gates then read as a clean
    pass. Generalized from BILL-343, which pinned this for one file after
    BILL-340 and BILL-341 each fixed an instance of it elsewhere.
    """

    @pytest.mark.parametrize(
        "path", _all_skill_markdown(), ids=lambda p: str(p.relative_to(SKILLS_DIR))
    )
    def test_no_pcre_only_grep_extension(self, path):
        text = path.read_text()
        hit = re.search(r"grep\s+-\w*P\w*\b|\\K", text)
        assert not hit, (
            f"{path.relative_to(REPO_ROOT)} embeds a GNU/PCRE2-only grep "
            f"construct ({hit.group(0)!r}). BSD grep — the macOS default — "
            "rejects it with exit 2 and empty stdout, which gates downstream "
            "read as 'nothing found'. Use a BRE/ERE form instead."
        )
