...
checks, single tier escalation, and the G-failure human gate with a non-blocking
fleet.
...
5. G-failure: failure ledger + per-ticket spend line (or degraded-mode note) + the
   four-option menu (more attempts / another rewrite / salvage — human-only /
   abandon); the fleet keeps running independent tickets while G-failure pends.
...
def test_g4_menu_and_nonblocking_fleet(ref):
    ...
    for option in ("more attempts", "rewrite", "salvage", "abandon"):
        assert option in lowered, f"G-failure menu must include {option}"
