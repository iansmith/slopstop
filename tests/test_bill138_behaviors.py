...
@pytest.fixture(scope="module")
def doc_section(prompt_text):
    idx = prompt_text.find("# Documentation")
    if idx == -1:
        pytest.fail("plan-agent-prompt.md has no '# Documentation' section — cannot test its contents")
...
def test_agent_prompt_reads_findings_at_start(doc_section):
    assert "At start" in doc_section, \
        "Documentation section must have an 'At start' instruction covering what to read before beginning work"


def test_agent_prompt_reads_task_plan_at_start(doc_section):
    assert "task_plan.md" in doc_section, \
        "Documentation section must instruct agents to read task_plan.md at start"


def test_agent_prompt_writes_findings_during_work(doc_section):
    assert "During work" in doc_section, \
        "Documentation section must have a 'During work' instruction for writing findings as they occur"


def test_agent_prompt_findings_uses_named_agent_sections(doc_section):
...
def test_agent_prompt_writes_findings_immediately_not_only_at_end(doc_section):
...
def test_agent_prompt_progress_summary_uses_named_sections(doc_section):
    assert "summary" in doc_section.lower(), \
        "Documentation section must specify a named '## Agent <id> summary' section for progress.md writes"


def test_agent_prompt_progress_summary_covers_done_when(doc_section):
...
def test_agent_prompt_progress_summary_required_on_stop(doc_section):
