---
name: slopstop-effort-max
description: slopstop worker carrier at max reasoning effort. Not for model selection — the orchestrator passes `model` on the Agent() call and this file only fixes effort. Never invoked directly; :run selects it from the resolved tier.
effort: max
---
