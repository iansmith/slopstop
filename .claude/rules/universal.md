...
- **Claude `/code-review` CAN be invoked by an agent**, as `Skill(skill: "code-review", args: "<PR#>")`. **Corrected 2026-08-27**, and it replaces a bullet that asserted the exact opposite: that it carries `disable-model-invocation`, that *"a skill, a subagent, or a headless run cannot"* launch it, and that *"any call site that appears to invoke it is **inert**"* — citing a harness check dated 2026-08-09. Re-tested on 2026-08-27 by a subagent reviewing a consuming-project PR: the call was accepted, ran as forked execution, and returned real findings, among them four false claims in the authoring session's own doc comments. Whether the old bullet was wrong when written or the harness changed under it cannot be told apart from here; what is known is that it does not hold now, and that it stood unchallenged for eighteen days. Whether it cost anything in that window is unknown — nobody may have tried — so the case for fixing it rests on it being false and cheap to check, not on damage anyone can point to.
  ...
  - **Accepted is not completed — corrected 2026-09-07.** The 2026-08-27 check established that the *call* is accepted from a subagent. It did not establish that the review finishes there, and it does not: `/code-review` runs its angles as **background agents**, and inside an `Agent()` wrapper the wrapper ends its turn ("I'll wait for the background agents") before they return — that sentence comes back as the worker's result, and removing the wrapper's worktree kills the angles still reading it. Measured on a consuming-project ticket (SOP-589): four of eight angles lost, the merge gated on the five that survived. **Invoke it inline from a context that did not write the code** — in lean `:run` that is the orchestrator, since the `work` worker wrote the diff — and wait for every angle before reading findings. Wrapping it in `Agent()` still satisfies the isolation half of this section while silently failing the review half, which is the worse failure: it reports a review that did not happen. Two stale claims about this one skill in six weeks; the durable test is whether the *findings* arrive, not whether the call does.
